...
const getDetalleVentas = async (req, res, id) => {
  try {
    const [result] =
      await connection.query(`SELECT detalle_ventas.idDetalleVenta, detalle_ventas.idVenta, ventas.total, detalle_ventas.idArticulo, articulos.nombre, cantidad FROM detalle_ventas 
        INNER JOIN ventas ON detalle_ventas.idVenta = ventas.idVenta
        INNER JOIN articulos ON detalle_ventas.idArticulo = articulos.idArticulo WHERE detalle_ventas.idVenta =${id}`);
...
const deleteDetalleVentas = async (req, res, id) => {
  try {
    const [result] = await connection.query(
      "DELETE FROM detalle_ventas WHERE idDetalleVenta=?",
      [id]
    );
